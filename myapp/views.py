...
class MessageFromUser(graphene.Mutation):
    message  = graphene.String()
    success  = graphene.Boolean()
    messages = graphene.Field(MessageObject)
    
    class Arguments:
        input = MessageInputObject(required=True)
    ...
    def mutate(self, info, input):
        try:
            ...
            message_response = MessageObject(
                username = message_instance.username,
                email = message_instance.email,
                description = message_instance.description,
                lat = message_instance.lat,
                lng = message_instance.lng
            )
...
class DeleteMessage(graphene.Mutation):
    ...
    def mutate(self, info, id):
        try:
            ...
            return DeleteMessage(success=False, message='Failed to delete message')
...
class CreateProduct(graphene.Mutation):
    ...
    class Arguments:
        input = ProductInputObject(required=True)

    def mutate(self, info, input):
        # Input Validation: Check if the name and price are valid
        if not input.name:
            raise GraphQLError("Invalid input: Name is required.")
        
        if not input.price or input.price <= 0:
            raise GraphQLError("Invalid input: Price must be a positive number.")
        ...
        try:
            ...
            product = Product.objects.create(
                name=input.name,
                price=input.price,
                discount_percentage=input.discount_percentage or 0,
            ...
            return CreateProduct(success=False, message=f"Error creating product: {str(e)}")
...
def upload_product_image(request):
    ...
    # Upload the image and link it to the product
    ...
    # Return the image URL along with the product info
    return Response({
...
class MessageQuery(graphene.ObjectType):
    all_message = graphene.List(MessageObject)
    def resolve_all_message(self, info):
        return Message.objects.all()
    
class MessageCountQuery(graphene.ObjectType):
    message_count = graphene.Int(description='Number zote za message zilizotumwa')
